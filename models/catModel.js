...
const getCat = async (catId, next) => {
    try {
        const [rows] = await promisePool.execute('SELECT coords, cat_id, wop_cat.name AS name, weight, birthdate, filename, wop_user.name AS ownername FROM wop_cat INNER JOIN wop_user ON owner = user_id WHERE cat_id = ?', [catId]);
...
const updateCat = async (cat, userId) => {
    try {
        if (userId === 0) {
        ...
        await promisePool.execute('UPDATE wop_cat SET name = ?, weight = ?, owner = ?, birthdate = ? WHERE cat_id = ? AND owner = ?', [cat.name, cat.weight, cat.owner, cat.birthdate, cat.id, userId]);
        return rows.affectedRows === 1;
...
const deleteCat = async (catId, userId) => {
    try {
        ...
        const [rows] = await promisePool.execute("DELETE FROM wop_cat WHERE cat_id = ? AND owner = ?", [catId, userId]);
